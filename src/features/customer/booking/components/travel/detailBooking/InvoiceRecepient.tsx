import { CheckCircle, FileDigit, ScanLine, Map } from "lucide-react";
        ...
        <div id="invoice-section">
            ...
            <div className="bg-white p-8 rounded-2xl border relative overflow-hidden">
                ...
                <div className="grid grid-cols-2 gap-8 py-6">
                    ...
                    <div className="text-right">
                        <p className="text-sm text-gray-500 font-semibold mb-1">Date of Issue</p>
                        <p className="font-medium text-gray-800">{payment.payment_date ? format(new Date(payment.payment_date), "dd MMMM yyyy") : 'N/A'}</p>
                        <p className="text-sm text-gray-500 font-semibold mt-2 mb-1">Status</p>
                ...
                <table className="w-full text-left">
                    <thead>
                        <tr className="bg-gray-50">
                            <th className="p-3 text-sm font-semibold text-gray-600 rounded-l-lg">DESCRIPTION</th>
                            <th className="p-3 text-sm font-semibold text-gray-600 text-right rounded-r-lg">AMOUNT</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr className="border-b">
                            <td className="p-3 pt-4 font-medium text-gray-800">
                                {booking.travel_package?.name} ({booking.pax_option?.pax} pax)
                            </td>
                            <td className="p-3 pt-4 text-gray-800 font-semibold text-right">{formatRupiah(payment.total_amount)}</td>
                        </tr>
                    </tbody>
                </table>
                    ...
                    <div className="w-full max-w-sm space-y-3">
                        <div className="border-t-2 border-dashed pt-3">
                            <div className="flex justify-between items-baseline">
                                ...
                                <span className="font-bold text-xl text-gray-900">{formatRupiah(payment.total_amount)}</span>
                            </div>
                        </div>
                    </div>
                </div>
