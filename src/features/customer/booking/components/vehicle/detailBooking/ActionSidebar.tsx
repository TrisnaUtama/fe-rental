...
const SummaryRow = ({ label, value }: { label: string; value: string }) => ( <div className="flex justify-between items-center text-sm"><span className="text-gray-500">{label}</span><span className="font-semibold text-gray-800">{value}</span></div>);
...
export function BookingActionsSidebar({ booking, days, onPayment, onPrint, onGiveRating }: BookingActionsSidebarProps) {
      ...
      case "PAYMENT_PENDING":
        return (
          <Card className="border-0 shadow-none">
            ...
            <CardContent>
              <p className="text-sm text-gray-600 mb-4">Your booking has been accepted. Please complete the payment to confirm your vehicle.</p>
      ...
      case "COMPLETE":
        return (
          <div className="space-y-4 pt-4 border-t">
            <StatusInfoBlock variant="success" icon={CheckCircle} title="Booking Complete" description="Thank you for choosing our service!" />
        ...
        <Card className="rounded-2xl shadow-sm">
          <CardContent className="p-6 space-y-4">
            ...
            <div className="space-y-2 pt-2">
                <SummaryRow label={`Price per day`} value={formatRupiah(booking.booking_vehicles!.reduce((sum, { vehicle }) => sum + Number(vehicle.price_per_day),0))}/>
                <SummaryRow label={`Rental duration`} value={`${days} Day(s)`} />
            </div>
            <div className="border-t pt-4 mt-2">
                <div className="flex justify-between items-center"><span className="text-base font-bold text-gray-900">Total Price</span><span className="text-2xl font-extrabold text-blue-600">{formatRupiah(booking.total_price!)}</span></div>
            </div>
            {renderActionCard()}
          </CardContent>
        </Card>

        {isActionLocked && (status === 'CONFIRMED' || status === 'RESCHEDULED' || status!.startsWith('REJECTED')) && (
            <div className="flex items-center gap-2 justify-center text-xs text-slate-500 p-2">
                <Info className="w-4 h-4" />
                Actions are locked within 24 hours of booking start.
            </div>
        )}
      </div>
