...
export default function UpdatePromoForm() {
  const { id: promo_id } = useParams<{ id: string }>();
  const { accessToken } = useAuthContext();
  const { data } = usePromoById(promo_id!, accessToken!);
  const { mutateAsync: updatePromoAsync, isPending: isCreating } =
    useUpdatePromo(accessToken || "");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updatePromoAsync({id:promo_id!, data:payload});
      toast.success("Promo Update successfully", {
        description: `Promo with code "${form.code}" successfully updated.`,
      });
      navigate(-1);
      resetForm();
       if (data?.success) {
        toast.success("Success", {
          description: `Successfully updated ${data.data.code} Destination`,
        });
        resetForm();
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <Card className="shadow-lg">
          <CardHeader>
            ...
            <CardDescription>
              Update your promo for customer satisfaying.
            </CardDescription>
          </CardHeader>
        </Card>
