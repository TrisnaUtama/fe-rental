...
export default function TravelPackagePopularityReport() {
  ...
  const travelPackagePopularity:
    | ITravelPackagePopularityReport
    | null
    | undefined = popularityResponse?.data;

  const popularityData: ITravelPackagePopularityItem[] = useMemo(() => {
    if (!travelPackagePopularity) return [];
    return Object.entries(travelPackagePopularity).map(([id, data]) => ({
      id,
      name: data.name,
  ...
  const totalPackagesTracked = popularityData.length;
  const totalBookingCount = popularityData.reduce(
    (sum, item) => sum + item.bookingCount,
    0
  );
  const totalRevenueGenerated = popularityData.reduce(
  ...
  const handlePrint = () => {
    ...
    const printContent = `
      <html>
      <head>
        <title>Travel Package Popularity Report</title>
      ...
      <body class="bg-white">
        <div class="mx-auto">
          <header class="flex justify-between items-start pb-4 border-b-2 border-gray-800 mb-6">
            <div>
              <h1 class="text-3xl font-bold text-gray-900">Travel Package Popularity Report</h1>
          ...
          <section class="grid grid-cols-3 gap-4 mb-6">
            <div class="bg-indigo-50 border border-indigo-200 rounded-lg p-4 no-break">
              <p class="text-sm font-medium text-indigo-800">Total Packages Tracked</p>
              <p class="text-2xl font-bold text-indigo-900">${totalPackagesTracked}</p>
            </div>
            ...
            <div class="bg-green-50 border border-green-200 rounded-lg p-4 no-break">
              <p class="text-sm font-medium text-green-800">Total Revenue Generated</p>
              <p class="text-2xl font-bold text-green-900">${formatCurrency(totalRevenueGenerated)}</p>
            </div>
          </section>

          <section class="mb-6">
            <h3 class="text-lg font-semibold text-gray-800 mb-4">Travel Package Popularity Details</h3>
            <div class="overflow-x-auto">
              <table class="w-full border border-gray-300">
                <thead class="bg-gray-100">
                  <tr>
                    <th class="border border-gray-300 px-4 py-3 text-left text-sm font-medium text-gray-700">Package Name</th>
                    <th class="border border-gray-300 px-4 py-3 text-center text-sm font-medium text-gray-700">Booking Count</th>
                    <th class="border border-gray-300 px-4 py-3 text-center text-sm font-medium text-gray-700">Total Revenue</th>
                  </tr>
                </thead>
                <tbody>
                  ${popularityData
                    ...
                    <tr>
                      ...
                      <td class="border border-gray-300 px-4 py-3 text-sm text-center text-gray-600">${
                        item.bookingCount
                      }</td>
                      <td class="border border-gray-300 px-4 py-3 text-sm text-center text-gray-600">${formatCurrency(item.totalRevenue)}</td>
                    </tr>
                  `
                    )
                    .join("")}
  ...
  const popularityColumns: Column<ITravelPackagePopularityItem>[] = [
    {
      header: "Package Name",
      accessor: "name",
    ...
    },
    {
      ...
      accessor: (item) => formatCurrency(Number(item.totalRevenue)),
      className: "text-center",
      cellClassName: "text-center text-green-700 font-medium",
    },
  ];
  ...
  return (
    <div className=" min-h-screen p-4 sm:p-6 lg:p-8 font-sans">
      <div className="max-w-7xl mx-auto">
        <header className="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">
              Travel Package Popularity Report
            </h1>
            <p className="text-md text-gray-600 mt-1">
              Insights into your most popular travel packages and their revenue.
            </p>
          </div>
          <div className="flex items-center gap-2 mt-4 sm:mt-0 no-print">
            ...
            <button
              onClick={handlePrint}
              disabled={isLoading || !travelPackagePopularity}
        ...
        {/* Report Data Display */}
        {travelPackagePopularity && popularityData.length > 0 ? (
          <div ref={printRef}>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              <StatCard
                icon={<Plane size={22} />}
                title="Total Packages Tracked"
                value={totalPackagesTracked}
                color="indigo"
              />
              <StatCard
                icon={<Hash size={22} />}
            ...
            <Card className="p-6 mb-6">
              <h2 className="text-xl font-bold text-gray-900 mb-4">
                Travel Package Popularity Details
              </h2>
              <DataTable
          ...
          <Card className="p-12 text-center text-gray-500">
            ...
            <p>
              No travel package popularity data available for the selected date
              range.
            </p>
