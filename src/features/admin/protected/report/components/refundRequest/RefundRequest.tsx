...
import { useRefundRequestsReport } from "../../hooks/useReport";
import type { DateRange, IRefundReportItem, Refund_Status } from "../../types/report";
...
import { RefundStatusBadge } from "../ui/RefundStatusBadge";

export default function RefundRequestReport() {
  ...
  const refunds: IRefundReportItem[] = refundsResponse?.data || [];
  
  const filteredData = useMemo(() => {
    if (!Array.isArray(refunds)) return [];
    return refunds.filter((r: IRefundReportItem) => {
  ...
  const reportSummary = useMemo(() => {
      ...
      return { totalRequests, totalRefunded, pendingRequests };
  }, [filteredData]);
  ...
  const handlePrint = () => {
    ...
    const printContent = `
        <html><head><title>Refund Request Report</title><script src="https://cdn.tailwindcss.com"></script><style>@import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&display=swap'); body { font-family: 'Inter', sans-serif; -webkit-print-color-adjust: exact; } @page { size: A4; margin: 1.5cm; } tr { page-break-inside: avoid !important; } thead { display: table-header-group !important; } table { border-collapse: collapse; }</style></head>
        <body class="bg-white"><div class="mx-auto"><header class="flex justify-between items-start pb-4 border-b-2 border-gray-800"><div><h1 class="text-3xl font-bold text-gray-900">Refund Request Report</h1><p class="text-sm text-gray-600">Generated on: ${new Date().toLocaleString('id-ID', { dateStyle: 'full', timeStyle: 'short' })}</p></div><div class="text-right"><h2 class="text-lg font-semibold text-gray-800">Your Company Name</h2><p class="text-xs text-gray-500">Jl. Jenderal Sudirman No. 123, Jakarta</p></div></header>
        <section class="grid grid-cols-3 gap-4 my-6">
            ...
            <div class="bg-green-50 border border-green-200 rounded-lg p-4"><p class="text-sm font-medium text-green-800">Total Refunded (Completed)</p><p class="text-2xl font-bold text-green-900">${formatCurrency(reportSummary.totalRefunded)}</p></div>
  ...
  const columns: Column<IRefundReportItem>[] = [
    { header: 'Request ID', accessor: 'id', cellClassName: 'font-mono text-gray-700' },
    { header: 'Booking ID', accessor: 'bookingId', cellClassName: 'font-mono text-gray-600' },
    { header: 'Request Date', accessor: (item) => new Date(item.request_date).toLocaleDateString("id-ID", { day: '2-digit', month: 'short', year: 'numeric'}), cellClassName: 'text-gray-600' },
    { header: 'Reason', accessor: 'reason', cellClassName: 'text-gray-800' },
    { header: 'Amount', accessor: (item) => new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR" }).format(Number(item.refund_amount)), className: 'text-right', cellClassName: 'text-gray-800 text-right font-semibold' },
    { header: 'Status', accessor: (item) => <RefundStatusBadge status={item.status} />, className: 'text-center', cellClassName: 'text-center' },
  ];
  ...
  return (
    <div className="min-h-screen p-4 sm:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto">
        <header className="mb-8"><h1 className="text-3xl font-bold text-gray-900">Refund Request Report</h1><p className="text-md text-gray-600 mt-1">Review, filter, and print all refund requests.</p></header>
        
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
            <StatCard icon={<Hash size={20} />} title="Total Requests" value={reportSummary.totalRequests} color="blue" />
            <StatCard icon={<DollarSign size={20} />} title="Total Refunded (Completed)" value={new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR" }).format(reportSummary.totalRefunded)} color="green" />
        ...
        <Card className="p-4 mb-6">
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 items-end">
                ...
                <div><label htmlFor="status-filter" className="block text-sm font-medium text-gray-700 mb-1">Status</label>
                    <select id="status-filter" value={status} onChange={(e) => setStatus(e.target.value as Refund_Status | "")} className="w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 capitalize transition-all">
        ...
        <footer className="text-center mt-6 text-sm text-gray-500"><p>Menampilkan {filteredData.length} dari {Array.isArray(refunds) ? refunds.length : 0} total permintaan refund.</p></footer>
      </div>
    </div>
  );
}
